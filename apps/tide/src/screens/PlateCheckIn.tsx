 ...
 * land on by tapping through it. Keeping that as a type-level fact (rather
 * than merely a UI convention) is what lets `draft.kind` below skip the
 * `'skipped'` branch entirely — `estimatePlateKcal`'s null-for-skipped path
 * genuinely never runs against a value this screen produces. */
...
function defaultDraft(): PlateComposition {
  return {
...
export function PlateCheckIn({ onNavigate }: PlateCheckInProps) {
  const [draft, setDraft] = useState<PlateComposition>(defaultDraft);
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-lg flex-col gap-6 px-4 pb-12 pt-safe-top">
      ...
      <TextAction onClick={() => void handleSkip()} className="self-start">
        Log a skipped meal
      </TextAction>
    </div>
  );
