...
// Run with: node scripts/generate-icons.mjs (from apps/tide/), or
// `npm run icons`.
//
// Ported verbatim from apps/runway/scripts/generate-icons.mjs (increment 9)
// — same rasterization approach, same output layout, only ROOT/BG_COLOR
// adjusted for this app. See that file if this one's own comments feel
// thin; nothing about the APPROACH changed in the port, only the source
// SVGs and the one color constant below.
//
// Why a script instead of a one-off image tool: the SVGs in assets/ are the
// only committed source of truth for Tide's icon. Regenerating from them is
// what keeps every density in sync after a motif tweak, instead of
...
// rather than by rasterizing the background SVG a second time. Also must
// match values/ic_launcher_background.xml (the legacy launcher's background
// color) — three places, kept in sync by hand; see icon-background.svg's
// own header comment.
