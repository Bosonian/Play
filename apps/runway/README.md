...
- **No scheduled notifications.** A departure needs staged alarms because getting ready happens away from the phone, ahead of time, without anyone necessarily watching a countdown. A task doesn't have that gap: it starts deliberately, at a desk, with the live TaskRun screen already open — the live screen is the entire instrument here, not a fallback for when an alarm doesn't fire. If tasks ever need a reminder that a deadline is approaching before the screen is opened, that's new scope, not an oversight in this version.
