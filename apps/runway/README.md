...
**Armed deadlines and the "To arm" shelf are also visible without opening the app (anti-rot increment 3, 0.39.0)** — see "Home-screen widgets" below for the "Runway Tasks" widget.

## Automatic arrival

The manual "I'm at the building" tap always works, but three more ways to trigger it exist — all stamp `arrivedAt` with the exact same write the button does, so nothing downstream (calibration, History, the arrival checklist itself) can tell which path recorded it. None is a replacement for the button: any can fail to fire (network timing, a Bluetooth event a sleeping OS never delivers, the phone's screen being off, a routine that doesn't run), so it stays as the honest fallback either way.

**Car Bluetooth disconnect (0.44.0), the most accurate of the three when it's available — see "Car Bluetooth transit" below for the full mechanism.** Field report, verbatim: "the Wi-Fi getting detected in the parking lot itself ... We can maybe start ... once car bluetooth disconnects." Hospital Wi-Fi often associates the moment the phone is in range — in the car park, before Deepak has actually parked, gotten out, and started walking — which stamps `arrivedAt` too early and inflates the first arrival step's measured time. The car's Bluetooth session ending (parked, doors opened, the head unit drops the link) is a closer proxy for "the walk-in has actually begun," and this app already listens for exactly that event for transit timing. This path takes PRECEDENCE over Wi-Fi where both fire: if Wi-Fi already stamped an early `arrivedAt` and no arrival step has been checked off yet, a later car-Bluetooth disconnect re-anchors `arrivedAt` FORWARD to the more accurate time — but never once a step is checked (that would retroactively recompute an actual already lived against the earlier anchor), and never backward. See `src/lib/bluetoothArrival.ts`'s `resolveCarArrival` for the exact rules.
...
**The DISCONNECT half also marks arrival (0.44.0).** Same app-open cadence, plus every time the app regains focus (`App.tsx`'s visibilitychange resume hook) — a second, independent reader of the same ring, `src/lib/bluetoothArrivalSync.ts`'s `syncBluetoothArrival`, looks at just the disconnect timestamps and decides whether one should start or re-anchor a `'left'` departure's `arrivedAt`. See "Automatic arrival" above for the precedence over Wi-Fi and why the disconnect is the more accurate signal. Choosing a car on Settings now does double duty — drive timing AND arrival detection — and the section's caption spells that out; "Choose car" itself hasn't moved.
...
**Also not yet verified, specific to arrival (0.44.0):** whether One UI reliably delivers the disconnect broadcast to a fully killed app the same way it delivers the connect one — this feature assumes it does, on the strength of the same broadcast-exemption fact above, but that's untested for the disconnect direction specifically. And separately: the real-world gap between "he's parked and getting out" and "the head unit actually drops the Bluetooth link" is unmeasured — some head units hold the ACL connection open for 10-30 seconds after ignition off, which would mean this signal, too, lags the true moment by a small amount, just a much smaller one than Wi-Fi's car-park-wide association range. Neither caveat is a reason not to ship this: it's strictly additive (Wi-Fi and the manual tap remain as fallbacks, nothing is removed), and even an imperfect disconnect signal is very likely closer to the truth than an SSID that can associate from across a parking lot.
...
Some mornings there's no bath. As of 0.51.0, the current step of a running departure's PREP checklist has a quiet "Skip this step" text action (checklist card, and — arithmetic permitting, see StepFocus.tsx's own width-check comment — the step-focus countdown's top row) beside "Done earlier." Skipping stamps `checkedAt` exactly like a real check-off — that's what lets `currentStepAnchor` move on to the next step and `computeProjection` drop the skipped step's minutes from the remaining plan, both for free — but also sets `DepartureStep.skipped`, which is the one thing that changes downstream: a skipped step reads as "skipped," never "done," everywhere a completed run's steps are shown (the checked-steps list, the running-departure editor's locked rows), and it is invisible to learning.

**A step Deepak didn't do must contribute nothing to its own estimate — not near-zero, nothing.** Before this increment, a skip could only be simulated by checking a step off immediately, which would record a real (near-zero) actual and drag that step name's learned estimate toward zero one skipped morning at a time — the same class of statistical corruption 0.49.0/0.50.0 fixed for an *added* step polluting a *neighbour's* estimate. `deriveChain` (`calibration.ts`) now excludes a skipped step from the `StepActual` list it emits while still advancing its internal cursor past the skip's timestamp — so the step immediately AFTER a skip is still measured from the skip instant, not from whatever step came before it. `learning.ts` and `autoLearn.ts` both consume that same excluded output (`deriveStepActuals`), so neither needed a change of their own to inherit the exclusion.

**Un-skip is un-check, reusing the exact same write.** A skipped step, still visible in the checked-steps list (dimmed, dashed placeholder instead of a ticked box, "· skipped" instead of a struck-through name — never a checked checkbox for something that never happened), restores to unchecked-and-unskipped with one tap, through the same `toggleStep` path a completed step already uses to un-check itself.

**Scoped to prep steps only, this increment.** `deriveChain`'s fix is shared machinery, so an arrival step's actuals get the same protection automatically the moment arrival steps grow their own skip control — but no such control exists yet; arrival steps behave exactly as before.
...
**Daily shape** (added 0.41.0): an optional day-sized floor beside that same weekly math — a target number of sprints for the day (1–4) and an optional rest day, set on ExamSetup below Study blocks. Surfaces as **"Today: 1 of 3 sprints."** (or **"Rest day."** on the configured day) directly under the weekly progress bar on the exam overview, as **"{n} of {target} today."** on the post-sprint screen, and on the Prüfung widget. **This number is deliberately decoupled from the pace projection above it and feeds no part of the ready-by/required-pace math** — 3 sprints × 50 min × 6 days is 15 h/week, honestly less than most required paces, and this line exists to get a day started, not to imply the week is on track. The ready-by date and required-pace line stay exactly as honest with or without a daily shape set.
...
Three 3×1 widgets, all fed from the same snapshot mechanism (see below).

**Prüfung widget** shows **"Ready by {date}"** (coloured calm/tight/late, the same thresholds as the exam overview), then a thin weekly progress bar (added 0.40.0 — mirrors ExamOverview.tsx's own weekly bar, the ONE progress bar sanctioned elsewhere in this app; sky while this week's logged hours are still short of the required pace, emerald once they've met or passed it, and a genuinely empty track at 0% rather than a hidden bar), then the daily-shape line (added 0.41.0 — **"Today: 1 of 3 sprints."** or **"Rest day."**, slate-grey until met then emerald, hidden entirely when no daily target is set), then this week's hours ("This week: 1.5 of 6.5 h"), then the exam anchor ("Exam window opens ..." or "Exam ..."), smallest and dimmest, last. Tapping it opens straight to the Prüfung overview.
...
**"Runway Tasks" widget** (added 0.39.0, the last anti-rot increment — armed deadlines and parked captures visible without opening anything) shows up to three lines: the soonest armed deadline among 'planned'/'running' tasks — task name (bold) and **"due 16:00 · start by 14:30"**, shortening to just **"due 16:00"** once that start-by moment has already passed — or **"No armed deadlines."** when no such task exists. A third line, **"{N} armed · {M} to arm"**, appears whenever either number is nonzero: N is the total count of planned/running tasks (the headline task, when shown, is included in N, not counted separately), M is the count of tasks parked on Home's "To arm" shelf. Unlike the departure widget, an overdue task's deadline is never hidden or expired away — the soonest deadline wins even when it's in the past, because a missed deadline is the single most urgent fact this widget exists to surface, not a stale one to suppress. Tapping anywhere opens Home (never a specific task).

**The honest staleness design:** none of the three widgets polls or recomputes anything on its own between app opens. Everything any of them shows is a snapshot the app pushes explicitly — after a sprint ends, after the exam/topics/a milestone are saved, after a departure is saved/started/left/abandoned/removed or its live travel time drifts, or after a task is saved/started/finished/abandoned/captured/armed — never on a timer. Between those moments, the Prüfung widget keeps its display current in only one narrow way: the "Ready by" date is calendar-slid forward day-by-day from the day the snapshot was generated (both ends anchored to local midnight, the same whole-calendar-day rule the app's own ready-date math uses — see `src/lib/widgetSnapshot.ts`), so the *date itself* stays right even while the app is closed, and agrees with what the app would show if opened right now. Everything else on that widget — the underlying pace, the remaining hours, whether this week's line still describes the current week — is frozen at whatever it was the last time the app ran, and the this-week line (plus the progress bar beside it, 0.40.0) disappears once the real calendar has moved past the week it was computed for. The daily-shape line (0.41.0) gets its own one-day version of that same guard: it disappears once the real device date has moved past the calendar day the snapshot was generated on, so a widget left unrefreshed overnight never shows yesterday's sprint count under today's label. The departure widget takes a stricter line on staleness: a departure fact doesn't have a graceful "still roughly true" reading the way a ready-by date does, so rather than slide anything forward, the widget re-checks the real clock against the snapshot's appointment time on every redraw and falls back to "No departure planned." the moment more than an hour has passed since that appointment — a stale "Klinik 14:30" from a departure that's since been left, missed, or removed while the app was closed never lingers on the home screen. The tasks widget takes neither approach: it has no staleness handling at all, because an overdue deadline isn't a stale fact to correct for — it's exactly the thing this widget is supposed to still be showing.

**Widget expiry/redraw is evaluated roughly every 6 hours, plus every app open** — a passed departure appointment can linger up to that long when the app stays closed before falling back to "No departure planned.". This is an accepted tradeoff, not a bug: forcing a faster system-level redraw tick would spend battery keeping a home-screen widget current while nobody's looking at it, for a staleness window that already self-heals the moment the app is next opened.

**Add a widget:** long-press the home screen → Widgets → Runway → pick Prüfung, Next departure, or Runway Tasks.
...
- **In-app APK download + self-install** — 0.42.0's update check (Home's "Update available" card, `src/lib/updateCheck.ts`; see CHANGELOG.md) opens the browser to `runway-latest.apk` and stops there; downloading the APK from inside the app and driving Android's installer directly would need the `REQUEST_INSTALL_PACKAGES` permission and a native install-package call, a meaningfully bigger native surface than a version check earns in its first increment.
