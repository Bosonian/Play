...
// the existing key-value `settings` table" treatment as
// liveTravelSettings.ts's Routes API key — two more rows don't earn a
// dedicated table.
 ...
 * Deepak's own Play repo, not a dedicated feedback repo, so field reports
 * work with zero setup beyond generating a token. Settings.tsx shows this
 * as the field's placeholder so the default is visible even before it's
 * ever been typed. */
 ...
 * reportSync.ts is the only reader. Mirrors readLiveTravelConfig's
 * "read-through, never touch the two rows directly" pattern
 * (liveTravelSettings.ts), except a report config has no "enabled" gate of
 * its own: a present, well-shaped token IS the enable switch (reportSync's
 * step 1 — no token, return silently).
 */
