...
## 0.44.0
- **The car Bluetooth disconnect now marks arrival, not just drive timing.** Field report, verbatim: "the Wi-Fi getting detected in the parking lot itself ... I don't see the option at all to say which bluetooth belongs to the car so that it at least knows that I am still in the car and not away from the car. We can maybe start ... once car bluetooth disconnects." Hospital Wi-Fi commonly associates the moment the phone is in range — in the car park, before Deepak has actually parked, gotten out, and started walking — which stamps `arrivedAt` too early and inflates the first arrival step's measured time (this is the same mechanism 0.43.1 diagnosed but didn't fix, in that release's Part C). The watched car's Bluetooth disconnect (parked, doors opened, the link drops) was already being recorded for drive-timing purposes (0.36.0) but never used for arrival — this increment wires that second use.
  - **`src/lib/bluetoothArrival.ts`** (new): pure `resolveCarArrival(departure, disconnectEventsMs, now)` — given a candidate departure and the watched car's raw disconnect timestamps, decides whether a disconnect should start or re-anchor `arrivedAt`, or do nothing. Only a disconnect strictly after `leftAt` and within the same 12h `ARRIVAL_MATCH_WINDOW_MS` freshness gate `externalArrival.ts`'s `runway://arrived` path already uses (now exported and reused, not re-declared, so the two windows can't silently drift apart) counts; of several qualifying disconnects, the EARLIEST wins. With `arrivedAt` still null, the disconnect starts the arrival phase outright. With `arrivedAt` already set (Wi-Fi, manual tap, or an earlier car-Bluetooth sync) and every arrival step still unchecked, a LATER disconnect re-anchors `arrivedAt` FORWARD — this is the fix for the car-park-early Wi-Fi case, removing exactly the dead "parked to building" minutes the field report describes. Once any arrival step is checked, the anchor is frozen — moving it after that point would retroactively recompute an actual already lived against the old anchor, the same "reality already happened, don't launder it" principle `applyReanchor` (Runway.tsx) already applies to `appointmentAt`. An `arrivedAt` already at or after the disconnect is left alone. 13 new tests: no disconnects, wrong status, empty/undefined `arrivalSteps`, no `leftAt`, disconnect at/before `leftAt`, a stale disconnect outside the window, fresh start, forward re-anchor, no re-anchor once a step is checked, no re-anchor when `arrivedAt` is already at-or-after the disconnect (including an exact-equal boundary case), and earliest-of-several-disconnects wins.
  - **`src/lib/bluetoothArrivalSync.ts`** (new): `syncBluetoothArrival()`, the Dexie/Capacitor orchestrator around the pure function above — same split `transitSync.ts` (the sibling reading the exact same native ring for drive timing) already uses. No-ops early, before any native read, when no car is configured (`WATCHED_DEVICE_ADDRESS_SETTING` absent). Candidate query mirrors `selectArrivalCandidate`'s shape (`status === 'left'`, `arrivalSteps` non-empty, soonest-appointment tiebreak) but deliberately does NOT also require `arrivedAt == null` — an already-arrived candidate is exactly what the re-anchor branch needs to see. Logs a distinct sentence for each outcome: `"Arrival detected via car Bluetooth: {name}."` on a fresh start, `"Arrival re-anchored to car Bluetooth: {name}."` on a re-anchor. Never throws (try/catch, fire-and-forget, matching every other startup materializer's contract); no haptic (unlike the Wi-Fi poll and the manual button, this can run from a background resume with the phone in a pocket — a buzz there would be unexplained noise, not feedback).
  - **`src/main.tsx`**: `void syncBluetoothArrival();` added to the startup sequence, right after `syncTransitEvents()` (both read the same native ring; adjacency here is for readability, the ordering between the two isn't load-bearing).
  - **`src/App.tsx`**: the existing visibilitychange resume hook (previously `refreshDayGauge()` + a "App resumed." log line only) now also calls `syncBluetoothArrival()`. This is the half that actually matters for the reported scenario: the car disconnects while Runway is closed (phone in a pocket on the walk in), so a cold-start-only sync would miss it until the NEXT full app restart — the resume hook is what lets a mid-day reopen pick it up immediately.
  - **`src/screens/Settings.tsx`**: the "Car Bluetooth" section's caption rewritten to name what "Choose car" now does — drive timing AND arrival — since the field report's other complaint was pure discoverability ("I don't see the option at all to say which bluetooth belongs to the car"). The "Choose car" action itself is unchanged in position; only the caption below it changed. Exact new copy: *"Your car's Bluetooth connect-to-disconnect measures each drive AND marks your arrival — the disconnect (you parked and got out) is when the walk-in begins, more accurate than Wi-Fi in the car park. Samsung may stop delivering Bluetooth events to apps it puts to sleep — exclude Runway from battery optimization if this stops working."*
  - **`README.md`**: "Automatic arrival" gains a third method (car Bluetooth disconnect, its precedence over Wi-Fi, and the never-move-a-checked-step guard); "Car Bluetooth transit" gains a paragraph on the disconnect's second use and an added "not yet verified" caveat specific to arrival (below).
  - **UNVERIFIED on device, stated plainly per CLAUDE.md's honesty rule, not glossed over**: (1) whether One UI reliably delivers the Bluetooth DISCONNECT broadcast to a fully killed app the same way it delivers the connect one — the broadcast-exemption fact this rests on is Android-documented, but untested here for the disconnect direction specifically; (2) the real gap between "parked and getting out" and "the head unit actually drops the ACL link" — some head units hold the connection 10-30 seconds past ignition-off, so this signal likely still lags the true moment by a small amount, just a much smaller one than Wi-Fi's car-park-wide association range. Neither caveat blocks shipping this: it's strictly additive — Wi-Fi and the manual "I'm at the building" tap remain exactly as they were, nothing is removed, and even an imperfect disconnect signal is very likely closer to the truth than an SSID that associates from across a parking lot.
- **The Learning page can no longer render blank (field report #16, the second attempt).** v0.43.1 added an error boundary, but the blank page persisted — because it was never a *throw*. Root cause, verified against Dexie's own source: `dexie-react-hooks`' `useLiveQuery` returns `undefined` both while loading AND permanently if the underlying `liveQuery` observable never resolves — and Dexie deliberately does NOT surface a `DatabaseClosedError`/`AbortError` through `observer.error`, so a closed-connection hiccup (common on Android background→resume, which this user's activity log shows constantly) leaves a query silently stuck at `undefined` with nothing for a boundary to catch. The old `if (!departures || !tasks || !sprints) return null;` turned that stuck state into a permanently blank screen with no way back.
  - **`src/screens/Learning.tsx`** restructured to a stated invariant: the header (with its back button) renders in every branch, loading is distinguished from loaded-empty ("Loading…" vs "Nothing learned yet."), and every derived-data section is wrapped in `safely` (below) so a throw in any one computation degrades that section alone — logged as `"Learning section failed: {which}."` and surfaced once as "Some learning data could not be shown." — instead of blanking the whole screen. It can show empty, it can show partial; it can never show blank.
  - **`src/lib/safely.ts`** (new): a generic `safely(compute, fallback, label, onError)` try/catch wrapper, pure and unit-tested (4 tests) with `onError` threaded in as a callback rather than hardcoded to `logEvent`, so it stays IndexedDB-free and testable.
  - No specific throwing computation was found — all the pure functions are already well-guarded — so this is hardening against the stuck-`undefined` mechanism, not a fix at a single source; if a real throw ever does occur, its `label` now names the failing section in the activity log.
  - `npm run typecheck && npm run test && npm run build` all pass from `apps/runway` — 676 tests (659 baseline + 13 for `bluetoothArrival.test.ts` + 4 for `safely.test.ts`). No new native plugin (the receiver and the ring it feeds already existed, 0.36.0) — `npx cap sync android` skipped.
  - `versionCode 63` / `versionName "0.44.0"`.

## 0.43.1
- **Field report #16, two threads: a blank Learning screen, and "highly inflated" arrival-step times.** Investigated as two independent questions rather than one bug, because nothing in the trace tied them together.
  - **The blank page (Part A) — not reproduced.** Every pure function the Learning screen renders through (`learningReport`, `guessPairs`/`biasFromPairs`/`globalBias`, `slipMinutes`/`slipTrend`/`medianMinutes`, `measuredPaceHoursPerWeek`, `transitMeasurementSummaries`) was exercised against the reporter's plausible data shape — departures with arrival steps and a real late slip, single-element and empty arrays, a sprint with `startedAt === endedAt`, a topic with `estimatedHours: 0`, malformed/`null` transit-settings JSON — and none threw, produced `Infinity`, or hit an unguarded `undefined` access. Every non-null assertion on this screen (`medianMinutes(...)!` in `medianOfPairField`) is only ever reached once the caller has already confirmed a non-empty array, so it's provably safe, not just apparently safe. The one thing that COULD throw on bad transit-settings JSON (`Object.entries(null)`, if the stored value parses to `null`) is already inside Learning.tsx's own try/catch. No commit has touched Learning.tsx or any of its five dependency files since before 0.41.1, so whatever produced the reported blank page either needed a data shape this investigation didn't hit, or has since stopped occurring for an unrelated reason (e.g. a Dexie/IndexedDB transient) — this release cannot claim to have found and fixed a specific throw, and says so rather than shipping a guess.
  - **The real fix regardless: this app had zero error boundaries.** A blank page in a Capacitor WebView IS an uncaught render-time exception — with no boundary anywhere, React unmounts the whole tree and the WebView shows nothing, with zero on-device trail. That gap is real independent of whether Part A's specific throw was ever found, and it's what turned whatever actually happened into an undiagnosable "blank page" report instead of a readable one.
    - **`src/ui/ErrorBoundary.tsx`** (new): a class component (React 18 has no hook equivalent for `getDerivedStateFromError`/`componentDidCatch`) wrapping every screen's render in `App.tsx`. On catch: a calm full-screen message — *"Something broke on this screen."* / *"The error is saved to the activity log. Tap to go home."* — plus a **Go home** button that resets the boundary and navigates to Home. `componentDidCatch` logs `logEvent('lifecycle', "Screen error: {message}.")`, fire-and-forget, same never-block-the-user contract every other `logEvent` call site follows. The exported `truncateErrorMessage(message, maxChars = 120)` bounds the logged message to the activity log's one-line-per-event shape before an arbitrarily long third-party error string can blow it up — 6 new tests (unchanged/truncated/custom-limit/empty/exact-boundary/one-past-boundary). This app has no jsdom/React Testing Library precedent, so the boundary's actual catch/render path is verified by reading rather than a render test; `truncateErrorMessage` is the one piece of real, pure logic inside it, and that's what's covered directly.
    - **`src/App.tsx`**: `renderScreen()`'s output (already wrapped in the fade-in `<div key={screen.name}>`) is now wrapped again by `<ErrorBoundary onReset={() => setScreen({ name: 'home' })}>`. Navigating home after a crash remounts the fade wrapper under a new `key`, which gives the boundary a fresh instance for free — no separate reset plumbing needed beyond the button's own `onReset` call.
    - **What this does and doesn't fix, stated plainly**: it doesn't fix whatever screen-specific bug throws — that still needs its own diagnosis, same as any other bug. What it fixes is turning every FUTURE render crash, on any screen, from a silent white screen into a recoverable message with a logged event to read afterward.
  - **The arrival-time inflation (Part C) — diagnosed, not a code defect, no fix shipped.** Traced against the reporter's actual activity log for the day (`arrivedAt` 07:37:14, "Get to Spind" checked at 07:39:32 then again at 07:41:33, "Get uniformed"/"Get to Station" checked seconds apart at 07:45:49/07:45:50).
    - **(a) The Spind double-check.** `DepartureStep`/arrival-step rows store exactly one `checkedAt` — there's no way to represent "checked, then unchecked, then rechecked," only the final value. `deriveStepActuals`' arrival chain (`calibration.ts`) confirmed to use that final `checkedAt` (07:41:33), which bakes in the ~2-minute re-tap gap (vs. the ~2-minute gap the first tap at 07:39:32 would have produced). This is arguably correct, not a bug: 07:41:33 IS when he finalized the step. One caveat worth stating precisely, since the field-report brief assumed otherwise: 0.43.0's double-tap gate (`StepFocus.tsx`) only guards the whole-glass step-focus surface's tap-to-advance gesture, not the small per-row checkbox in the checked-arrival-steps list (`Runway.tsx`'s `checkedArrival.map`) — that checkbox was explicitly scoped OUT of the 0.43.0 fix as "a small, aimed target, the opposite failure mode." If the Spind re-tap happened through that checkbox (plausible, given the ~2-minute gap between taps — too slow to be the 350ms double-tap pattern anyway), the double-tap gate does not prevent a repeat of this specific pattern going forward. Flagged, not fixed — extending a confirmation gate to a small aimed checkbox is a real UX tradeoff (it would slow down every ordinary correction-uncheck, not just this one) worth a deliberate decision, not a drive-by change in a diagnosis release.
    - **(b) The arrival-to-first-step gap — the likelier real inflation, and not a code defect.** `deriveStepActuals` anchors the whole arrival chain at `arrivedAt` (confirmed by reading, not assumed), so the entire 4m19s between the Wi-Fi-triggered `arrivedAt` stamp (07:37:14) and the first arrival-step check-off (07:41:33) is attributed to "Get to Spind" — exactly as the "gap between two check-offs belongs to the later one" rule the whole chain is built on (`calibration.ts`'s own header comment) says it should. If the hospital Wi-Fi routine fires the moment the phone associates — plausibly at the car park, before he's physically walking toward the Spind — that dead time gets folded into "Get to Spind"'s learned estimate, inflating it. This is inherent to the measurement source (Wi-Fi-connects-early, real-work-starts-later), not a mis-anchoring bug: there is no other timestamp in the data that could split "walked from car to building" from "at the Spind, getting changed." The honest correction the app already has is the arrival step's own "Done earlier" backdate — flagged here as the answer, not a code change.
  - 659 tests total, up from 653 (6 above, `ErrorBoundary.test.ts`).
  ...
  - `versionCode 62` / `versionName "0.43.1"`.

## 0.43.0
- **A pocket brush finished a real step.** Field report #14, verbatim directive: "prevent accidental touches for step completion." The step-focus countdown's whole-screen tap was built deliberately aim-free (the wet-hands/shower case — a small "done" button is a bad target when hands aren't free), and that same aim-freeness meant ANY tap anywhere checked the current step off and advanced — including a phone pulled out of a pocket, which is exactly how a real departure step got falsely finished. The fix keeps the surface whole-glass and aim-free (still usable one-handed, still usable blind) but raises the bar from one tap to a confirmed **double-tap**: a single tap now only flashes a teaching hint, *"Double-tap to check off."*, and produces zero haptic feedback (a pocket brush must stay silent even to the phone itself — a buzz on a false-positive first tap would be a smaller version of the same problem). Only a second tap landing within 350ms of the first checks the step off and advances, haptic included, exactly as before.
  - **`src/lib/doubleTap.ts`** (new): `DOUBLE_TAP_WINDOW_MS = 350` and pure `isSecondTap(lastTapAtMs, nowMs)` — true when a prior tap timestamp exists and `nowMs` falls within the window of it. Deliberately clock-free and stateless like `focusTone.ts`: the caller (StepFocus) owns the timestamp and all resets. 6 new tests: within window, exactly at the boundary (inclusive), just past it, 10s later, `null`, and a zero-gap tap.
  - **`src/screens/StepFocus.tsx`**: a `lastTapAtRef` (a `useRef`, not state — writing it must never itself trigger a re-render) holds the last unconfirmed tap's timestamp; `isSecondTap` is the entire debounce, so no timer is needed to arm or expire it — a tap that fails the check just becomes the new stored timestamp, which is what makes a tap 10s after the last one read as a fresh first tap with the hint again, with zero extra bookkeeping. A confirmed double-tap resets the ref to `null` (so a stray third tap can't chain into a false second one) and calls the caller's `onTap` exactly as the old single-tap version did — the haptic lives inside that callback (`advanceFocusAfterCheck`/`advanceArrivalFocusAfterCheck` in Runway.tsx, TaskRun.tsx's own version), so this change needed zero edits in either caller. The hint text reuses the exact slot/style the "Starts when the steps before it are done." line already occupied below the countdown digits (`text-sm text-slate-500`, mutually exclusive with it — one is current-step-only, the other excludes the current step) rather than adding a second instruction spot, and auto-hides after 1.4s via one `setTimeout`, cleaned up on unmount.
  - **Deliberately unchanged**: the back chevron and "Done earlier" (backdate) — both stay single-tap. They're small, aimed targets a stray brush is unlikely to land on at all, unlike the whole-glass surface the double-tap gate exists for; commented in place as the distinction. The ordinary step-list checkboxes everywhere else in the app (Runway.tsx's step cards, TaskRun.tsx's unit cards) are untouched and stay one-tap — those are deliberately SMALL, aimed targets already, the opposite failure mode from the whole-glass focus screen, and the increment brief for this fix explicitly scoped it to StepFocus only. Landscape verified by reading, not a separate code path: the tap handler lives on the same `fixed inset-0` container in both orientations, landscape only repositions children via CSS, so one change covers portrait, landscape, departures, arrival steps, and task units alike.
  - **Old single-tap phrasing, grepped for and checked everywhere**: the only two places in the repo describing the old behavior were this file's own `onClick`/wet-hands code comment (updated above) and CHANGELOG.md's 0.18.0 entry ("Tap anywhere: step done…") — that entry is left as-is on purpose, since it's an accurate historical record of what 0.18.0 actually shipped, not a current instruction; rewriting past changelog entries to match present behavior would misrepresent that release's own history. No README.md wording claimed single-tap explicitly (its "step-focus tap-through" mentions are behavior-agnostic), so nothing there needed changing.
  - **Escalation path if stray completions continue**: a proximity-sensor gate (only accept a tap while the phone is confirmed out of a pocket/face-up) was considered and declined for this increment — it needs a native plugin this app doesn't have yet, and 350ms double-tap is a proven, zero-dependency pattern (it's how "like" gestures work) that should already close the reported failure mode. If double-tap alone isn't enough in practice, proximity/orientation gating is the next lever, not a reason to loosen the window.
  - 653 tests total, up from 647 (6 above).
  - `npm run typecheck && npm run test && npm run build` all pass from `apps/runway`. No native code touched — `npx cap sync android` skipped.
  - `versionCode 61` / `versionName "0.43.0"`.

## 0.42.1
- **The first stamped release actually stamps.** 0.42.0's release name came out as mangled comment text: build.gradle's LOUD NOTE gained the words versionCode/versionName, and the workflow's unanchored `grep -m1` matched the comment line above the real fields. Both extractions are now anchored to the line start, so a comment can never win. The in-app parser read the mangled name as "no signal" (by design — never a false update), so no device was misled; the protocol simply starts one version later than planned: 0.42.0 installs see their first update card for this release.

## 0.42.0
- **The app now notices its own updates.** Every release before this one ended the same way: build, push, then manually remember to open GitHub, find the `runway-latest` release, download the APK by hand. Runway now checks for a newer build itself and offers the download from Home — v1 scope is check + a Home card + opening the browser to the APK; it deliberately stops short of downloading and self-installing the APK in-app (that needs Android's `REQUEST_INSTALL_PACKAGES` permission and a native install-package call, a bigger and more sensitive native surface than a version check earns in its first increment — cut here, not forgotten).
  - **`.github/workflows/runway-apk.yml`**: the release publish step now stamps the `runway-latest` release's `name` as `Runway v{versionName} ({versionCode})`, extracted from `android/app/build.gradle` in the existing "Compute release metadata" step (plain grep/sed, no new dependency) rather than hardcoded — the fixed name "Runway (latest build)" it used before carried no machine-readable version at all. Tag (`runway-latest`) and asset name (`runway-latest.apk`) are unchanged. **Caveat that matters**: every release published before this one still carries the old unstamped name, and republishing only happens on the next push to `apps/runway/` — this app's own update checker can only ever recognize a release the *new* workflow step produced, which means the very first version capable of telling Deepak "an update exists" is 0.42.0 itself. A device running 0.42.0 will correctly stay silent until 0.42.1 (or later) is published; it cannot retroactively notice that 0.42.0 itself was newer than whatever it replaced.
  - **`src/lib/updateCheck.ts`** (new): `parseReleaseName(name)` parses the stamped shape, `null` for anything else (most importantly every pre-0.42.0 release) — a `null` is "no signal available", never treated as "up to date" or as "an update", a distinction stated explicitly in the function's own doc comment because getting it backwards would either nag forever on old releases or go silent forever on a currently-unstamped one. `checkForUpdate(force?)` is throttled via a `lastUpdateCheckAt` settings row (minimum 6h between real network checks; Settings' "Check now" bypasses it via `force: true`), fetches `GET api.github.com/repos/Bosonian/Play/releases/tags/runway-latest` unauthenticated (public repo, no token needed — same CapacitorHttp-native/fetch-web split as `routesApi.ts`/`reportSync.ts`, for the same CORS-bypass reason), and compares the parsed `versionCode` against `APP_VERSION_CODE`. Newer writes an `availableUpdate` settings row (`{version, versionCode}` JSON); same/older/unknown clears it. **Never throws** — offline, GitHub rate-limiting the unauthenticated 60/h quota, a malformed response, or a Dexie write failure are all caught and logged once (`lifecycle` category — justified in the file's own comment: an update check is a startup/background concern about the app itself, not any of the other seven categories, and didn't earn a whole new category for two log lines). **Judgment call, flagged**: `lastUpdateCheckAt` is stamped even after a FAILED attempt, not only a successful one — the alternative (stamp on success only) sounds safer but means a phone offline for days would retry on every single app open instead of backing off, defeating the throttle's purpose; the tradeoff is that a first-ever failed check waits up to 6h before trying again. Called fire-and-forget from `main.tsx`, beside the other startup calls. 20 new tests (7 `parseReleaseName` cases, 3 `parseAvailableUpdate` cases, 10 `checkForUpdate` cases covering throttle/force/newer/same/older/unknown/network-failure/non-2xx/Dexie-failure) — `db`/`eventLog` mocked, same precedent as `eventLog.test.ts`/`reportSync.test.ts`, no real IndexedDB.
  - **`src/screens/Home.tsx`**: a new card leads the screen — above the first-run card, the permission banners, and the create-departure/create-task buttons, because an available update is meta-app, not day content. Exact copy: *"Update available: v{remote}."* / *"You have v{APP_VERSION}."*, with a primary **Download** button (`window.open` to `.../releases/download/runway-latest/runway-latest.apk`, `_blank`, plus a `logEvent`) and a **Not now** `TextAction`. "Not now" is a session-only dismissal — a module-level `Set<versionCode>`, same idiom `dismissedSuggestions` already uses on this screen — so it reappears on the next app open (an update reminder that could be permanently silenced defeats itself) but won't re-nag on every render within the same session, and dismissing one version doesn't suppress a *newer* one discovered later the same session. **Re-guarded at render** against `APP_VERSION_CODE`, not just trusted off the settings row: a phone that just installed the exact build the row was advertising still has that stale row sitting in Dexie until the next 6h-throttled check clears it, so without this guard a just-updated app could tell Deepak to update to the version he's already running.
  - **`src/screens/Settings.tsx`**: new "Updates" section (last, after Activity log) — `Version {APP_VERSION} ({APP_VERSION_CODE}).` plus a **Check for updates** `TextAction` that calls `checkForUpdate(true)` and shows inline feedback: `Up to date.` / `Update available: v{X} — see Home.` / `Could not check. Try again later.` (`text-sm`, calm — the error line uses `text-amber-400`, matching the app's existing "soft failure, retry later" tone rather than `text-red-400`'s harder failure tone).
  - **Housekeeping**: `versionCode`/`versionName` in `android/app/build.gradle`, `APP_VERSION`, and the new `APP_VERSION_CODE` in `src/lib/appVersion.ts` are now THREE hand-maintained values that must move together — both files' LOUD comments are updated to say so; nothing new enforces it (still a v1.5 candidate, see README.md).
  - 647 tests total, up from 627 (20 above).
  - `npm run typecheck && npm run test && npm run build` all pass from `apps/runway`. The CI workflow edit is **verified by reading only** — this environment cannot run GitHub Actions, so the next push is its actual live test.
  - `versionCode 59` / `versionName "0.42.0"`.
